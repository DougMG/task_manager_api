...
RSpec.describe 'Task API' do
  ...
  describe 'GET /tasks' do
    context 'when filter params not sent' do
      before do
    ...
    context 'when filter params and sorting is sent' do
      ...
      let!(:other_task_1) { create(:task, title: "Fix a door", user_id: user.id )  }
  ...
  describe 'POST /tasks'do
    ...
    context 'when the params are valid'do
      ...
      it 'when returns status code 201' do
        expect(response).to have_http_status(201)
      end

      it 'saves the tasks in the database' do
        expect( Task.find_by(title: task_params[:title]) ).not_to be_nil
      end

      it 'returns the for created task' do
